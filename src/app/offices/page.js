...
export default function OfficesPage() {
  ...
  const handleSaveOffice = async (e) => {
    ...
    if (!formData.name.trim() || !formData.code.trim()) {
      setFormError('Please fill in Deploying Area Code and Deploying Area Name.');
      return;
    }
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-emerald-600 text-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-100">
                    Code: {selectedOfficeForDetails.code} {selectedOfficeForDetails.floor ? `• Location: ${selectedOfficeForDetails.floor}` : ''}
                  </p>
                </div>
              </div>
              <button
